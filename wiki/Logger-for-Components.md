...
# TODO duplication here and further down

For lightning component developers, the included `logger` lwc can be used in other lwc & aura components for frontend logging. Similar to `Logger` and `LogEntryBuilder` [Apex classes](./Logger-for-Apex), the lwc has both `logger` and `logEntryBuilder` classes. This provides a fluent API for JavaScript developers so that they can chain multiple function calls.

For lightning component developers, the `logger` lwc provides logging capabilities to lightning web components and aura components, with very similar functionality that is offered [in Apex](./Logger-for-Apex). Simply embed the `logger` lwc in your component, and call the desired logging methods within your code.
...
TODO include examples of calling `setError()` and other JS builder methods
